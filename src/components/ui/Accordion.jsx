...
export default function FaqAccordion() {
  ...
  return (
    <section className="max-w-3xl mx-auto px-6 py-16 ">
      <h1 className="text-4xl font-semibold mb-3 text-center text-gray-900">
        Have any questions?
      </h1>
        ...
        {faqData.map((item, index) => (
          ...
          >
            <button
              ...
              className="w-full flex justify-between items-center px-8 py-5 text-left text-lg font-semibold focus:outline-none hover:bg-[#f8f4f0] transition-colors duration-300"
            >
              <span>{item.question}</span>
              <span className="text-3xl text-gray-700 select-none">
                {openIndex === index ? <RxCross2 /> : <BiPlusCircle />}
              </span>
            </button>

            <div
              className={`px-8 pb-6 text-gray-700 text-base transition-[max-height] duration-300 ease-in-out overflow-hidden ${
                openIndex === index ? "max-h-96" : "max-h-0"
              }`}
            >
              {openIndex === index && <p className="leading-relaxed">{item.answer}</p>}
            </div>
          </div>
        ))}
      </div>
